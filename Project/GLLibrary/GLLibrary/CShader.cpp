...
const char* sky_frag = "#version 430\n\n"\
"#define SHADOW_MAX 12\n"\
"#define LIGHT_MAX 20\n"\
"uniform vec3 lightPos[LIGHT_MAX];\n"\
"uniform vec3 lightDir[LIGHT_MAX];\n"\
...
"	if(texColor.a * alpha <=0) discard;\n"
"	for (int i = 0; i < LIGHT_MAX; i++) {\n"\
"		if (lightType[i] != 1) continue;\n"\
"		vec3 color = lightAmbientColor[i] + lightDiffuseColor[i];\n"\
"		out_color = (texColor * vec4(color, Diffuse.w * alpha));\n"\
"		break;\n"\
"	}\n"\
"}";

const char* sky_frag_f = "#version 430\n\n"\
"#define SHADOW_MAX 12\n"\
"#define LIGHT_MAX 20\n"\
"uniform vec3 lightPos[LIGHT_MAX];\n"\
"uniform vec3 lightDir[LIGHT_MAX];\n"\
...
const char* swordtrail_frag_d = "#version 430\n"\
"uniform sampler2D sampler;\n"\
"uniform vec3 Emissive;\n"\
"in vec2 texCoord;\n"\
"in vec4 vertex_color;\n"\
...
"	texColor = texture2D(sampler, texCoord);\n"\
"	vec4 color = vertex_color * texColor;\n"\
"	out_color[0] = vec4(color.xyz + out_color[0].xyz,color.w);\n"\
"	out_color[1].w = 0.0f;\n"\
"	out_color[3].x = Emissive.x;\n"\
"	out_color[3].y = 0.0;\n"\
"	out_color[3].z = 1.0;\n"\
"	out_color[3].w = 1.0;\n"\
"};\n";
const char* swordtrail_frag_f = "#version 430\n"\
"uniform sampler2D sampler;\n"\
"uniform vec3 Emissive;\n"\
"in vec2 texCoord;\n"\
"in vec4 vertex_color;\n"\
...
"	texColor = texture2D(sampler, texCoord);\n"\
"	vec4 color = vertex_color * texColor;\n"\
"	out_color[0] = vec4(color.xyz + color.xyz + out_color[0].xyz,color.w);\n"\
"	out_color[1] = vec4(Emissive,1);\n"\
"};\n";

const char* swordtrail_frag = "#version 430\n"\
"uniform sampler2D sampler;\n"\
"uniform vec3 Emissive;\n"\
"in vec2 texCoord;\n"\
"in vec4 vertex_color;\n"\
...
"	texColor = texture2D(sampler, texCoord);\n"\
"	vec4 color = vertex_color * texColor;\n"\
"	out_color = vec4((color.xyz+Emissive.xyz) + out_color.xyz,color.w);\n"\
"};\n";
...
CShader::CShader(std::string type) {
	static std::map<std::string, std::vector<std::vector<const char*>>> list = {
		...
		},
		{
			...
			"SwordTrail",
			{
				{trail_vert,swordtrail_frag},
				{trail_vert,swordtrail_frag_f},
				{trail_vert,swordtrail_frag_d},
			}
		},
		{
			"GaussianBlur",
